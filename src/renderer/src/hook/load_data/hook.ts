...
export function useLoadData<T>() {
  ...
  const data: Ref<Array<{ item?: T }>> = ref([])
  let loading = false
  const loadTempData = (num: number) => {
    for (let i = 0; i < num; ++i) {
      data.value.push({})
  ...
  const clearExcessData = () => {
    const index = data.value.findIndex((e) => e.item === undefined)
    if (index !== -1) {
      const num = data.value.length - index
      data.value.splice(index, num)
    }
  ...
  async function loadData(num: number, load: () => Promise<T[]>, clear = true) {
    if (loading) return
    ...
    res.forEach((item, index) => (data.value[index + loadedAmount].item = item))
    loadedAmount += res.length
    clearExcessData()
  }
