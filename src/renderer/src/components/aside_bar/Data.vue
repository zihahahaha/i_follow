...
const activeClass = computed(() => {
  if (route.name === 'search_data') return linkStyle.active
  else return ''
})
function active(srcId: string) {
  if (route.name === 'manager_search_data' && route.params.metaSrcId === srcId) {
    return linkStyle.active
  } else return ''
}
watchEffect(() => {
  for (let view of state.views.value) {
    if (route.name === 'manager_search_data' && route.params.metaSrcId === view.srcId)
    ...
    >
      <div :class="[$style.link, activeClass]">
        <a @click="navigate" :class="linkStyle.link">
          ...
          <div :class="linkStyle.text">数据</div>
        </a>
